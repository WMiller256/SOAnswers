...
x = np.arange('2020-04-03T00:00Z', '2020-04-05T00:30Z', 60, dtype='datetime64[m]')
...
fill = [False if dt.time(8, 0, 0) < t.time() < dt.time(20, 0, 0) else True for t in 
		np.arange(x[0], x[-1]+60, 60, dtype='datetime64[m]').astype(dt.datetime)]
plt.tick_params(axis='x', rotation=90)
plt.fill_between(x, plt.ylim()[0], plt.ylim()[1], where=fill, fc='grey', alpha=0.2)
